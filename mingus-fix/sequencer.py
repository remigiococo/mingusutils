        ...
        for x in range(len(tracks)):
            ...
            if isinstance(instr, MidiInstrument):
                try:
                    ...
                    #i = 1
                    i = instr.instrument_nr # REMIGIO
                self.set_instrument(channels[x], i, bank)
                print "<<<", x, channels[x], i
            else:
                try:
                  self.set_instrument(channels[x], instr.instrument_nr, bank)
                except:	
                  self.set_instrument(channels[x], 1, bank)
        current_bar = 0
